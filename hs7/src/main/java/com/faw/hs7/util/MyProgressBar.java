...
public class MyProgressBar extends ImageView {
    ...
    public MyProgressBar(Context context, boolean isCreate) {
        super(context);
        init();
        width = (int) getResources().getDimension(R.dimen.hs7_default_progress_bar_width);
        height = (int) getResources().getDimension(R.dimen.hs7_default_progress_bar_height);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (defaultDrawable == null)
            defaultDrawable = getResources().getDrawable(R.mipmap.hs7_loading1);
        if (width == 0)
            width = this.getWidth();
        if (height == 0)
